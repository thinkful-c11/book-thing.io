...
app.get('/api/recommendation/:listid', passport.authenticate('bearer', {session: false}), (req, res) => {
  ...
  return knex('lists').where({id: req.params.listid}).then(results => {
    ...
    return knex('lists_to_users').where({list_id: req.params.listid, created_flag: true}).select('user_id').then(myUserID => {
      console.log(myUserID);
      return knex('lists_to_users').where({created_flag: true}).andWhereNot({user_id: myUserID[0].user_id}).select('list_id');
    });
  }).then(otherLists => {
    console.log("this is what i'm looking for: ", otherLists);
    otherLists.forEach(list => {
      values.push(list.list_id);
    });
    console.log(values);
    ...
    otherListsToReturn.forEach(otherList => {
      promises.push(knex('books_to_lists').where({list_id: otherList.id}).join('books', 'books.id', '=', 'books_to_lists.book_id').select('books.id', 'books.title', 'books.author', 'books.blurb').then(potentialRecBooks => {
    ...
    return knex('lists_to_users').whereNotExists(() => {
      this.select('*').from('lists_to_users').where({user_id: myUserID[0].user_id, list_id: recommendation.id, created_flag: false});
    }).insert({user_id: myUserID[0].user_id, list_id: recommendation.id, created_flag: false}).returning(['id', 'list_id', 'user_id', 'created_flag', 'liked_flag']);
  }).then(final_result => {
...
app.put('/api/lists/likes/:id', passport.authenticate('bearer', {session: false}), (req, res) => {
  return knex('lists').where('id', '=', `${req.params.id}`).andWhere({}).increment('likes_counter', 1).returning('likes_counter').then(_res => {
